    ...
    for (const file of files) {
        ...
        if (fileStat.isDirectory()) {
            ...
            if (remainingFilesInDir.length === 0 ||
                ...
                // Don't remove 'css' or 'js' directory in 'build-admin'
                if (filePath !== join(dirPath, 'css') && filePath !== join(dirPath, 'js')) {
                    await rmdir(filePath)
                }
            }
        } else if (filePath !== join(dirPath, 'css/admin.css') && filePath !== join(dirPath, 'js/admin.js')) {
